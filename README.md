...
- **本番環境 URL**: [https://push-two-chi.vercel.app/](https://push-two-chi.vercel.app/)
...
npm run dev
```
起動後、PCのブラウザで http://localhost:3000 にアクセスします。
...
1. **URL を開く**
   iPhone の Safari を開き、本番 URL (https://push-two-chi.vercel.app/) にアクセスします。
...
```plaintext
├── public/
│   ├── sw.js            # 通知の受信とクリックを制御する Service Worker
│   └── next.svg         # 通知用アイコン（仮）
└── src/
    └── app/
        ├── layout.tsx   # メタデータおよびマニフェストの紐付け
        ├── manifest.ts  # スマホアプリ化（PWA）に必須の定義ファイル
        └── page.tsx     # 通知許可の要求と、通知トリガーを持つメイン UI
```
